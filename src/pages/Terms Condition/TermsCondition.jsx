...
function TermsCondition() {
  ...
  useEffect(() => {
    console.log("Fetched termsData:", termsData);
    if (termsData) {
      const fetchedContent = 
        ...
        (typeof termsData?.data === 'string' ? termsData?.data : null) ||
        (Array.isArray(termsData?.data) ? (termsData.data[0]?.content || termsData.data[0]?.description) : null) ||
        termsData?.content ||
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await updateTermsAndConditions({ requestData: { content, description: content } }).unwrap();
      Swal.fire({
        icon: 'success',
        title: 'Saved',
  ...
  return (
    <>
      ...
      <div className=" bg-[#393d4a] rounded shadow p-5 h-full border border-[#4a5060]">
        {isFetching ? (
          <div className="text-white text-center py-10">Loading terms & conditions...</div>
        ) : (
          <ReactQuill
